...
public class RedisUtil {
    ...
    static {
        Properties redisProperty = PropUtil.getProperty("db.properties");
        if (null != redisProperty.getProperty("redis.pool.maxActive") && null != redisProperty.getProperty("redis.pool.maxIdle")
                && null != redisProperty.getProperty("redis.pool.maxWait") && null != redisProperty.getProperty("redis.ips")) {
            // 创建jedis池配置实例
            ...
            for (String ip : ips) {
                JedisShardInfo jedisShardInfo = new JedisShardInfo(ip.split(":")[0], Integer.valueOf(ip.split(":")[1]));
